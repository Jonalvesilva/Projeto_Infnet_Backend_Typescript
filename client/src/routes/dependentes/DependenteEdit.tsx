...
import { putDependente } from "../../api/dependentes/putDependente";
...
export function DependenteEdit() {
  const params = useParams();
  const navigate = useNavigate();
  const isLoading = useGlobalStore((state) => state.isLoading);
  const setIsLoading = useGlobalStore((state) => state.setIsLoading);
  const zo = useZorm("editDependente", createDependenteSchema, {
    async onValidSubmit(event) {
      event.preventDefault();
      const dependente = event.data;
      setIsLoading(true);
      const response = await putDependente(
        Number(params.id),
        Number(params.idDep),
        dependente
      );
